...
final case class Rule[I, O](rule: I => Validated[ValidationError, O]) extends MonadRules[I] with RuleExecutor[I, O] {

  def map[OO](f: O => OO): Rule[I ,OO] = super.map(this)(b => f(b))

  def flatMap[OO](f: O => Rule[I, OO]): Rule[I, OO] = super.flatMap(this)(b => f(b))
